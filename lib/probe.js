      ...
      Object.keys(data).forEach((parentSensorKey) => {
        if (data[parentSensorKey].type === 'ZLLPresence' && parseUId(data, parentSensorKey) === uId) {
          if (!hueSensor[data[parentSensorKey].name]) {
            hueSensor[data[parentSensorKey].name] = {temp: 0, lightlevel: 0};
          }

          hueSensor[data[parentSensorKey].name]['temp'] = convert(data[key].state.temperature / 100).toFixed(2);
        }
      });
    }
...
function getData() {
  const data = axios.get(hueUrl).then((response) => {
    return getSensors(response.data);
  }).catch((error) => {
    console.log(error);
  });
  return data;
